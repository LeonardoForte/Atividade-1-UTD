<?php
	...
	class Manager extends Connect{
		...
		public function insert_common($table, $data, $query_extra){
			...
			//$data[':user_cpf'] = ;
			//substituindo os parametros nomeados pelos verdadeiros valores, ex: ":name" por "Leonardo"
			foreach ($data as $key => $value){
		...
		public function select_special($tables, $relationships, $filters, $query_extra){
			...
			if(isset($filters)){
				...
				//substituindo os parametros nomeados pelos verdadeiros valores, ex: "?" por "Leonardo", com a ajuda da variável auxiliar
				$i = 1;
				foreach ($filters as $key => $value){
					//$parameters[":$key"] = $value;
		...
		public function update_common($table, $data, $filters, $query_extra){
			...
			//substituindo os parametros dos filtros nomeados pelos verdadeiros valores, ex: ":name" por "Leonardo"
			foreach ($filters as $key => $value){
				//$parameters[":$key"] = $value;
				$statement->bindValue(":$key", $value, PDO::PARAM_STR);
			}
